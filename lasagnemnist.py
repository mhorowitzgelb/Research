...
def build_mlp(input_var=None):
    l_in = lasagne.layers.InputLayer(shape=(None, 131), input_var=input_var)
...
def main(model='mlp', num_epochs=500):
    ...
    y_train = data[0][1].astype(np.int32)
    y_val = data[1][1].astype(np.int32)
    ...
    test_acc = 0
    test_batches = 0
    for batch in iterate_minibatches(X_test, y_test, 80, shuffle=False):
        inputs, targets = batch
        err, acc = val_fn(inputs, targets)
        test_err += err
        test_acc += acc
